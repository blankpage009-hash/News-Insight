...
마지막 갱신: 2026-07-27 (커밋 `b84f6c8` 배포 직후)
...
### 상황 한 줄
`gemini-3.5-flash-lite` 가 **Render 에서만 응답을 안 준다.** 완충장치는 넣어 뒀지만
(다른 모델로 우회) 근본 해결은 안 됐다. **빠르면서 Render 에서도 되는 모델을 찾아야 한다.**

### 확인된 사실 (같은 API 키, 2026-07-27 실측)

| 모델 | 내 PC | Render |
|---|---|---|
| `gemini-3.5-flash-lite` | 717ms 정상 | **응답 없음** (상한까지 대기) |
| `gemini-3.6-flash` | 정상(13초, 하루 20회) | **정상 동작 확인** |

→ Render→구글 경로가 통째로 막힌 게 아니라 **모델별 현상**이다. 이건 확실하다.
→ 배포 후 라이브 실측 : 27,893ms 로 **성공**(3.6-flash 로 우회). 완충장치는 작동한다.
→ 다만 3.6-flash 는 13초 + 하루 20회라 상시로 쓸 수 없다.

### 다음에 할 일 (사용자에게 제안해 둔 상태, 승인 대기)

1. **Render 에서 어떤 모델이 응답하는지 재는 진단 경로를 붙인다.**
   `/api/gemini-models?test=1` 처럼, **Render 서버가** 후보 모델들에게 짧은 요청("안녕")을
   하나씩 보내 보고 status·소요시간을 표로 돌려주게 한다.
   **로컬에서는 재현이 안 되므로 Render 에서 재는 수밖에 없다.**
   후보로 볼 만한 것 : `gemini-3.1-flash-lite`, `gemini-3.5-flash`, `gemini-2.0-flash-lite`,
   `gemini-3-flash-preview` (전체 목록은 `/api/gemini-models` 의 `usable`).
2. **작은 버그 같이 고치기** — 후보를 다 훑고 실패하면 `callGeminiModels` 의 `lastErr` 가
   목록 맨 끝 2.5 계열의 **404** 라서, 진짜 원인(429·무응답) 대신
   "쓸 수 있는 Gemini 모델을 찾지 못했습니다" 가 표시된다. 원인 판단을 방해한다.
3. 위에서 쓸 만한 모델을 찾으면 `MODEL_CANDIDATES` 1순위를 그걸로 바꾼다.

### 참고 — 3.6-flash 하루 한도는 오늘(2026-07-27) 소진됐다
측정하느라 20회를 다 썼다. 하루가 지나면 회복된다(구글 기준 시각이라 한국 시간 자정과는 다르다).
그전에 테스트하면 429만 보게 되니, 시작할 때 먼저 이걸로 회복 여부를 확인할 것 :

```
curl -s -X POST "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=$GEMINI_API_KEY" -H "Content-Type: application/json" -d "{\"contents\":[{\"parts\":[{\"text\":\"안녕\"}]}]}"
```
